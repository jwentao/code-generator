...
function formItemCompile(el) {
  if (el && Array.isArray(el.children) && el.children.length) {
    ...
    el.children.forEach(scheme => {
      ...
      if (config.labelWidth && config.labelWidth !== el.labelWidth) {
      ...
      htmlList.push(`<el-form-item ${labelWidth} ${label} prop="${scheme.__vModel__}" ${required}>
        ${tagDom}
      </el-form-item>`);
    });
    return htmlList.join('\n');
